package com.itheima.demo2_servletcontext;

import javax.servlet.ServletContext;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;

/**
 * @Author pzl
 * @Date 2022/11/7 16:53
 */
@WebServlet("/ServletDemo18")
public class ServletDemo18 extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String fileName1 = "a.jpg";
        String fileName2 = "b.png";

        // 获取文件的mime类型
        String mimeF1 = getServletContext().getMimeType(fileName1);
        String mimeF2 = getServletContext().getMimeType(fileName2);
        System.out.println("mimeF1 = " + mimeF1);
        System.out.println("mimeF2 = " + mimeF2);

        System.out.println("------------------");

        // 获得ServletContext对象
        ServletContext servletContext = getServletContext();
        // 获取全局参数
        String value = servletContext.getInitParameter("dkey");
        System.out.println("value = " + value);
    }

    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        doPost(request,response);
    }
}
